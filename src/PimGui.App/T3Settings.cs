...
public sealed partial class MainWindow
{
    ...
    private async Task<bool> ConfirmDownloadOriginAsync(Uri uri)
    {
        ...
        try { return await Dialog("Review download source", T("This package uses another download host. Continue only if you trust it") + "\n\n" + uri.GetLeftPart(UriPartial.Authority), "Continue").ShowAsync() == ContentDialogResult.Primary; }
    ...
    private UIElement BuildSourceEditor()
    {
            ...
            body.Children.Add(palette.Label("This source applies only to PyDeck, not pip or external terminals", 13));
            body.Children.Add(input);
            var reset = palette.Action("Restore official source", compact: true); reset.Click += (_, _) => input.Text = ""; body.Children.Add(reset);
    ...
    private UIElement BuildShebangEditor()
    {
            ...
            string selected = "py"; body.Children.Add(template);
            body.Children.Add(Choice(targets.ToArray(), selected, value => selected = value, "Python version"));
            bool changed = false;
            void RenderRules()
            {
                list.Children.Clear();
            ...
            add.Click += (_, _) => { try { ShebangRules.ValidateMapping(template.Text, selected); rules[template.Text] = selected; changed = true; RenderRules(); } catch (Exception ex) { ShowError(ex); } };
